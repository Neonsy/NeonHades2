...
import { auditArcanaSources, type ArcanaSourceAudit } from "./source-audit.js";

export interface ArcanaExporterPreflightIssue {
  readonly code:
...
export interface ArcanaExporterPreflight {
  readonly schema: "neodes2-arcana-exporter-preflight-1";
  readonly exporterVersion: string;
  readonly sourceAudit: ArcanaSourceAudit;
  readonly issues: readonly ArcanaExporterPreflightIssue[];
...
function validateLuaStructure(source: string): readonly string[] {
  const tokens = [...maskLua(source).matchAll(/[A-Za-z_][A-Za-z0-9_]*|[()[\]{}]/gu)].map(
    (match) => match[0],
  );
  ...
  for (const token of tokens) {
    if (token === "(" || token === "[" || token === "{") brackets.push(token);
      ...
      if (brackets.pop() !== closing[token]) errors.push(`Unbalanced ${token}.`);
    } else if (token === "function" || token === "if") blocks.push("end");
    else if (token === "for" || token === "while") {
      blocks.push("end");
      pendingDo += 1;
    } else if (token === "do") {
      if (pendingDo > 0) pendingDo -= 1;
      else blocks.push("end");
    } else if (token === "repeat") blocks.push("until");
    else if (token === "end" || token === "until") {
      const expected = blocks.pop();
      if (expected !== token) errors.push(`Unexpected ${token}, expected ${expected ?? "no closing keyword"}.`);
    }
  }
  if (brackets.length > 0) errors.push(`Unclosed bracket ${brackets.at(-1)}.`);
  if (blocks.length > 0) errors.push(`Unclosed Lua block expecting ${blocks.at(-1)}.`);
  if (pendingDo > 0) errors.push("A for or while statement has no do keyword.");
  return errors;
}
...
export async function preflightArcanaExporter(
  modDirectory: string,
  sourceAcquisitionDirectory: string,
): Promise<ArcanaExporterPreflight> {
  const resolvedModDirectory = resolve(modDirectory);
  const [mainSource, arcanaSource, manifestSource, sourceAudit] = await Promise.all([
    readFile(join(resolvedModDirectory, "main.lua"), "utf8"),
    readFile(join(resolvedModDirectory, "arcana.lua"), "utf8"),
    readFile(join(resolvedModDirectory, "manifest.json"), "utf8"),
    auditArcanaSources(sourceAcquisitionDirectory),
  ]);
  ...
  const issues: ArcanaExporterPreflightIssue[] = [];
  ...
  if (
    !mainSource.includes('"neodes2-arcana-runtime-manifest-1"') ||
    !arcanaSource.includes('schema = "neodes2-arcana-runtime-1"')
  ) {
    issues.push({
      code: "missing-runtime-schema",
      file: "main.lua or arcana.lua",
      detail: "Arcana runtime report or finalization schema is absent.",
    });
  }
  ...
  if (!sourceAudit.complete) {
    issues.push({
      ...
      detail: "The authoritative static Arcana source audit is incomplete.",
    });
  }
  return {
    schema: "neodes2-arcana-exporter-preflight-1",
