...
import { preflightArcanaExporter } from "./arcana/mod-preflight.js";
...
const defaultOutputDirectory = fileURLToPath(
  new URL("../.local/arcana-preflight/", import.meta.url),
);
...
async function main(): Promise<void> {
  ...
  if (arguments_.includes("--help")) {
    console.log(`Usage: pnpm arcana:preflight -- --source-acquisition <absolute-path> [options]
...
Options:
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (!knownOptions.has(argument)) {
      throw new Error(`Unknown Arcana preflight option: ${argument}`);
    }
    index += 1;
  }
  const result = await preflightArcanaExporter(modDirectory, sourceAcquisition);
  ...
  console.log(`Arcana exporter preflight complete.
Complete: ${result.complete}
Exporter version: ${result.exporterVersion}
Cards: ${result.sourceAudit.cards.length}
Grasp levels: ${result.sourceAudit.graspLevelCount}
...
main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "Unknown Arcana preflight failure.";
  console.error(`Arcana preflight failed: ${message}`);
  process.exitCode = 1;
});
